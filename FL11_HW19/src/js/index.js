...
function game(event) {
  if (checkClass(event.target, 'bet-image')) {
    let computerBet = getComputerBet();
    let playerBet = event.target.alt;
    let arrOfChoosen = document.querySelectorAll('.choosen-bets__result>img');
    let scorePoint = document.querySelectorAll('.score>span');
      ...
      if (
        ...
        if (scorePoint[0].innerHTML === '3') {
          setTimeout(() => {
            ...
            let imParent = document.querySelectorAll('.result-image')[0];
            imParent.innerHTML = '';
